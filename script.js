...
// Curated pair list for the picker (item 1). Grouped for the popup.
const PAIR_GROUPS = [
  {
    label: "Forex — Majors",
    pairs: [
...
let perfRange = "1M"; // 1D | 1W | 1M | 3M | ALL — shared by chart + stat cards
...
let selectedCalendarDateKey = null; // "YYYY-MM-DD" of the tapped calendar day, or null
...
let addRiskMode = "percent"; // "percent" | "usd" — unit of the f_risk field
let accountEditOpen = false;
let pendingAvatarFile = null;
let pendingAvatarPreviewUrl = null;

// ---------- money / trading math ----------
// No more guessing pip size or $-per-pip per instrument (that's what broke
// on gold). SL/TP are entered manually from the trader's own chart tool.
// Everything else is derived from ONE ratio specific to this trade:
// dollars risked ÷ price-distance to stop-loss = $ per unit of price move.
// That ratio is instrument-agnostic — works identically for forex, gold,
// indices, crypto, anything — because it's built from numbers the trader
// already supplied, not a lookup table we'd have to maintain and could get wrong.
...
// ------------------------------------------------------------
// The trader sets their own risk % / $ directly — that stays the
// source of truth. This is a second, independent estimate built
// from lot size × entry × stop, using the standard contract sizes
// both Exness and XM use on their regular MT4/MT5 symbols, so we
// can flag it if the lot size doesn't actually match the stated
// risk. One Settings toggle (Standard vs Cent) covers both
// brokers, since their contract sizes are identical.
const ACCOUNT_TYPE_KEY = "tb_accountType"; // "standard" | "cent"
...
// Every symbol listed under the "Crypto" group in PAIR_GROUPS above —
// derived from that single source of truth rather than a separate
// guessed ticker list, so any pair added to that group later gets
// manual-risk treatment automatically with nothing else to update.
// Crypto contract sizes are broker-defined and nothing like a
// standard forex lot, so these always skip the estimate and fall
// back to manual entry — regardless of ticker length.
const CRYPTO_PAIRS = new Set(
  (PAIR_GROUPS.find((g) => /crypto/i.test(g.label))?.pairs || []).map(
    ([symbol]) => symbol,
  ),
);
// Same idea for indices. Today every index symbol (US30, NAS100, ...)
// contains a digit, so it already misses the 6-letter forex regex
// below and falls back to manual entry on its own — but that's
// coincidental, not guaranteed. An all-letter 6-character index
// symbol added later (e.g. NASDAQ) would otherwise silently match
// the forex branch and get priced with a 100,000-unit contract size.
// Checking this set explicitly makes the exclusion deliberate.
...
function contractSizeForPair(pair) {
  ...
  if (p.includes("XAU") || p === "GOLD") return 100; // troy oz
  if (p.includes("XAG") || p === "SILVER") return 5000; // troy oz
  // Standard 6-letter forex pair, e.g. EURUSD, GBPJPY.
  if (/^[A-Z]{6}$/.test(p)) return 100000;
  return null; // other symbols — broker-defined, skip estimate
}
// ------------------------------------------------------------
// Live FX rates (for converting quote-currency risk into USD)
// ------------------------------------------------------------
// Primary source is fawazahmed0/currency-api served as a static JSON
// file off jsdelivr's CDN — edge-cached, so it typically resolves
// much faster than hitting a live API origin server per request.
// jsdelivr has occasional outages (per the project's own docs), so
// there's a same-data CDN mirror as a second attempt, and the old
// frankfurter.dev origin as a last resort if both CDNs are down.
// One request pulls every currency at once (base=USD), so this
// covers ANY pair, not just a fixed list of majors. Refreshed
// periodically while the app is open; if every source fails (e.g.
// offline), callers fall back to the old entry-price estimate rather
// than breaking the Risk field.
let fxRates = null; // { USD: 1, CAD: 1.3758, JPY: 149.82, ... }
let fxRatesFetchedAt = 0;
let fxRatesLoadFailed = false; // true only once every source has failed
const FX_REFRESH_MS = 10 * 60 * 1000; // 10 minutes
const FX_RETRY_AFTER_FAIL_MS = 15 * 1000; // retry sooner right after a failure
...
async function fetchJson(url, timeoutMs = 4000) {
  ...
  try {
    ...
    return null; // timed out, offline, CORS hiccup, etc.
  } finally {
    clearTimeout(timer);
  }
}

// Any Risk field currently open should quietly sharpen the moment a
// rate table lands (or notice a failure) without the trader touching
// anything — called after every refresh attempt, not just the first.
...
async function refreshFxRates() {
  ...
  // Every source failed — keep whatever table we already had (if
  // any); just flag it so the Risk field can tell the trader this is
  // a real failure, not just still loading, and retry sooner than
...
// Is a cross pair (needs a live rate, has no entry-price fallback)
// still waiting on the fetch, or did it actually fail? Distinct from
// "this pair will never auto-calculate" (crypto, indices, missing
// fields) — those should stay silent, not show a loading/error hint.
function liveRateWaitState({ pair, entry, slPrice, lotSize }) {
  ...
  if (quote === "USD" || base === "USD") return null; // never "waiting"
  if (fxRateFor(quote) !== null) return null; // already have it
...
function estimateRiskFromLot({ pair, entry, slPrice, lotSize }) {
  ...
  // rawRisk above is in the pair's QUOTE currency, not necessarily USD.
  // MT5 reports risk in the account's deposit currency (USD here), so
  // any pair not quoted directly in USD needs one more conversion step.
  const p = (pair || "").toUpperCase().replace(/[^A-Z]/g, "");
  const isMetal =
    p.includes("XAU") || p === "GOLD" || p.includes("XAG") || p === "SILVER";
  if (isMetal) return rawRisk; // Exness/XM quote metals in USD already

  if (/^[A-Z]{6}$/.test(p)) {
    const base = p.slice(0, 3);
    const quote = p.slice(3, 6);
    if (quote === "USD") return rawRisk; // EURUSD, GBPUSD, ... — already USD

    // Prefer the live rates table — it converts ANY quote currency,
    // not just USD-base pairs, and tracks the market instead of a
    // single stale entry price.
    const liveRate = fxRateFor(quote);
    if (liveRate !== null) return rawRisk / liveRate;

    // No live table yet (first load, offline, fetch still in
    // flight) — fall back to the old approximation, which only
    // works when USD is the base currency (USDJPY, USDCAD, ...).
    if (base === "USD") return rawRisk / entry;

    return null; // cross pair (EURJPY, GBPCHF, ...) with no live rate yet
  }

  return null;
}

// Item 4 — strip ONLY genuine floating-point noise from a closing
// price, never real decimal precision. This used to hard-floor to
// 3 decimal places, which silently truncated legitimate 4th/5th
// digits on pairs like EURUSD (1.16691 -> 1.166) and threw P&L off
// by whole pips — a $101.21 result became $127.60 with no visible
// sign anything was wrong, since the price *label* still showed the
// untouched value while the P&L math used the truncated one. This
// only cleans up float artifacts (1.1 + 0.2 -> 1.3000000000000003);
// it never rounds off a real digit the trader/broker actually meant.
function roundDown3(n) {
  if (n === undefined || n === null || Number.isNaN(Number(n))) return n;
  return parseFloat(Number(n).toPrecision(12));
}

// How many decimals a price value actually carries — e.g. 1.1704 -> 4,
// 1.17083 -> 5. Used below to figure out the precision a trade is
// being worked in, straight from what the trader themselves typed
// for entry/SL/TP — no pair lookup table, no guessing.
...
// A price DERIVED from a typed win/loss amount (via exitPriceFromPnl)
// is a real division result, not floating-point noise — 40 / 29000
// genuinely is 1.1690206896551725..., and toPrecision(12) lets all of
// that through untouched since there's no "noise" to strip. That's a
// real bug: it showed a trader a closing price with 11 decimals for a
// pair they entered to 4-5. This rounds a derived price to the same
// number of decimals the trader already used for this trade's own
// entry/SL/TP — inferred from their own input, not an instrument
// lookup table — so a derived price reads like a real, quotable price.
...
// Item — risk warning. No threshold existed anywhere; a 25% risk trade
// sailed through with zero friction. This doesn't block anything by
// itself (a trader might genuinely mean it) — it surfaces the number
// clearly and requires one extra confirmation past the danger line.
...
function fmtPrice(n) {
  ...
  // Show the price exactly as entered/stored — this only strips
  // floating-point noise (e.g. 1.1 + 0.2 -> 1.3000000000000003),
  // it never rounds a real decimal off a genuine price.
...
// Turns the raw Postgres check-constraint message into something a
// person can actually act on, instead of surfacing the DB's
// internal constraint name straight to the UI.
...
// ---------- theme (local UI preference only — not sensitive data) ----------
...
async function bootInner() {
  ...
  // Fire-and-forget: don't block the auth/loading flow on this. Any
  // Risk field open when a rate lands (or a failure is confirmed)
  // gets nudged to recompute on its own — refreshFxRates notifies
  // internally now, so this covers every periodic refresh too, not
  // just the first one.
  refreshFxRates();
  ...
  // needs a moment to parse them into a session. Wait for that before
  // deciding the person isn't logged in — otherwise this flashes the
  // login screen even though the link was valid.
  ...
  // getSession() only decodes the JWT this device already has cached —
  // it reflects whatever user_metadata (avatar, name) was current at
  // this device's last login/token-refresh, not necessarily what's
  // current on the server right now. That's exactly why a profile
  // picture changed on one device wouldn't show up on another until
  // its token happened to refresh. getUser() makes a live call to
  // Supabase and returns the actual current record — use that as the
  // source of truth whenever we can reach the network, and only fall
  // back to the cached session if the device is offline.
  ...
  // person continue into the app on their own terms, rather than
...
document
  .getElementById("verifiedContinueBtn")
...
async function loadProfileAndData() {
  ...
  // No profile row yet — happens for Google/OAuth sign-ins, since they
  // skip the manual signup form that used to be the only place a
  // profile got created. Create one on the fly instead of erroring out.
  if (profErr && profErr.code === "PGRST116") {
    ...
    if (createErr) {
      ...
      document.getElementById("loginError").textContent =
        "Could not set up your profile. " + createErr.message;
      return;
    }
    prof = created;
    profErr = null;
...
// Postgres numeric columns come back as strings — convert to numbers once, here.
...
document.getElementById("signupForm").addEventListener("submit", async (e) => {
  ...
  if (!data.session) {
    ...
    // link (which depends on redirect/domain config that's easy to
    // get wrong). The Confirm Signup email template must include
...
// ============================================================
// PAIR PICKER — inline dropdown attached directly under the field
// (no more overlay modal, so it can never render behind the Add/Edit
// Trade modal — that was the old z-index stacking bug).
// ============================================================
...
function setupPairDropdown(inputId, dropdownId, onPreviewUpdate) {
  ...
  function render(query) {
    ...
    if (!currentMatches.length) {
      ...
      // Empty query: show everything, grouped by category so the full
      // forex/metals/crypto/indices list stays scannable.
      let html = "";
      let lastGroup = null;
      ...
      // Searching: flat filtered list, no group headers.
    ...
    dropdown.querySelectorAll(".pair-option").forEach((el) => {
      el.addEventListener("mousedown", (e) => {
        ...
        // can select before the dropdown gets hidden by blur.
        e.preventDefault();
  ...
  input.addEventListener("blur", () => {
    // Small delay so a mousedown-selection above still lands first.
...
// Item 9 — empty states as guides, not dead ends: every zero-data
// screen now names what's missing and gives a direct way forward.
...
// The close-trade chooser only ever offers two exact prices: the
// trade's own recorded SL and TP, taken verbatim — no interpolation,
// no in-between "estimated" price. It exists purely so you don't have
// to retype the entry/SL/TP by hand to close at one of them; there is
...
// Item 5 — ongoing trade card now includes Edit/Delete (desktop buttons
// shown via .card-actions, mobile via swipe-to-reveal wrapper below).
...
// Mobile swipe wrapper: card sits in `.swipe-card-inner`, secondary
// actions (Edit/Delete) revealed underneath by translating the inner
// card left on touch drag. Close trade is NOT in here — it's the
// primary action for an ongoing trade and stays on the card face at
// every screen size (see .close-trade-cta), so it's never gated
// behind a swipe gesture nobody's been told exists.
...
// Item — weekly recap (last 7 days): net P&L, win rate, and most
// frequently traded pair among trades closed in the trailing week.
...
// Trade-level streak (consecutive wins/losses by close order) plus
// days-since-last-trade — a habit signal, not a P&L signal. Net
// P&L can be flat for a week and this still tells the trader
// something a stat-strip number can't: are they still showing up.
function streakStats() {
  ...
  for (const t of sorted) {
    ...
    if (type === null) break; // breakeven trade ends the streak count
    if (streakType === null) {
      streakType = type;
      streakCount = 1;
...
// Closed trades can be swiped left to reveal an Edit action so a
// wrong closing price can be fixed after the fact. Once a trade has
// been edited this way, the edit action is gone for good — tracked
// client-side since it's a one-time allowance, not a full edit
// history. A trade that's already had its one edit just renders as
// a plain (non-swipeable) row.
...
// ============================================================
// PERFORMANCE TAB (items 3 + 8) — visual chart, range-scoped stat
// cards (Total P&L / Win rate / Profit factor / Expectancy), plus
// the existing by-day/month/year breakdown below.
// ============================================================
...
// Mood tags only mean something in aggregate — a single "Revenge"
// trade tells you nothing. MOOD_TAG_MIN_SAMPLE keeps one-off tags
// out of the insight card so it doesn't call out noise as a pattern.
...
// ------------------------------------------------------------
// PERFORMANCE CALENDAR — day-by-day P&L heatmap. Traders think in
// trading days, not a single equity line, so this sits alongside
// the range chart rather than replacing it. Uses only --win/--sell
// (via color-mix, so it stays on the existing monochrome palette
// with no new brand colors) — intensity scales with that day's P&L
// relative to the biggest day in the visible month.
// ------------------------------------------------------------
function dateKey(d) {
...
// Lightweight vanilla-SVG cumulative P&L line chart — no external
// charting library, keeps the same dark/light theme via CSS variables.
...
function resolveAvatarUrl() {
  ...
  const meta = currentUser.user_metadata || {};
  // Your own upload lives under a key Google's OAuth sync never
  // touches, so it always wins once you've set one, regardless of
  // what your Google profile picture is or later changes to.
  if (meta.custom_avatar_url) return meta.custom_avatar_url;
  // No custom picture chosen yet — fall back to whatever the sign-in
  // provider supplied. Google sign-in doesn't always leave a copy at
  // the top level of user_metadata; identity_data on the linked
  // identity is the more reliable place to check.
...
function attachContentListeners() {
  ...
  if (editAccountForm) {
    editAccountForm.addEventListener("submit", async (e) => {
      ...
      // Picture and name are two fields on one form, saved together.
        ...
        try {
          ...
          const { data: updatedUser, error: authErr } =
            await sb.auth.updateUser({
              // A distinct field, deliberately NOT "avatar_url" — Google
              // re-syncs that key from your Google profile on every
              // Google sign-in, which was silently overwriting your
              // own upload back to Google's picture. This key is
              // something Google's OAuth sync has no reason to ever
              // touch, so a picture you choose here stays chosen.
  ...
  if (editBalanceForm) {
    editBalanceForm.addEventListener("submit", async (e) => {
      ...
      // Item 6 — a manual balance edit is a correction (deposit,
      // withdrawal, fixing a typo), not a trading result. Shift
      // initial_balance by the same delta so (balance - initial_balance)
      // — the number all-time performance is measured against — is
      // unaffected by this edit and still reflects trades only.
...
// ---------- item 7: profile picture (picked here, cropped, then
// uploaded on Save changes) ----------
...
// Every save/close/delete now confirms what happened instead of just
// silently re-rendering — errors already surfaced via showSaveError,
// but success had no signal at all until now.
...
// ---------- item 6: restart with a new starting balance ----------
...
// ------------------------------------------------------------
// Auto-fills Risk from lot size, entry & stop, and locks the field
// while a valid estimate exists (matches MT5's own number via the
// live FX-converted estimate). Falls back to a free-typed field for
// pairs we can't estimate (crypto, indices, cross pairs with no live
// rate yet).
// ------------------------------------------------------------
function syncRiskFromLot() {
  ...
  if (lotEstimate === null) {
    // Can't estimate for this pair (missing data, crypto/indices, etc.)
    // — hand the field back so the trader can type their own risk.
    riskInput.readOnly = false;
    if (fieldWrap) fieldWrap.classList.remove("locked");

    // A true cross pair (EURJPY, GBPCHF, ...) that just hasn't got a
    // live rate yet looks identical to a pair that will never
    // auto-calculate unless we say so — this tells them apart.
  ...
  // Display only — riskInput.value above still drives the actual
  // submit/preview logic in whichever single unit addRiskMode is set
  // to. This just also shows the other unit so the trader can read
  // both without needing the %/$ toggle while the field is locked.
...
// Converts whatever's currently in the field to the new unit as a
// bridge value; syncRiskFromLot() immediately overwrites it with a
// fresh auto-calculated number when a lot estimate is available. For
// pairs with no estimate (crypto, indices, ...) this conversion is
// what actually sticks, since the field stays free-typed.
...
// All fields that feed either the lot-size mismatch check or the
// trade preview share one listener set. f_risk previously only
// triggered updateAddPreview() — syncRiskFromLot() (the mismatch
// check) never re-ran when the trader typed their risk value, so
// in the normal fill order (pair -> entry -> SL -> lot -> risk) the
// warning could never actually fire: by the time Risk had a value,
// nothing was listening for it anymore.
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    // Item 2 — checklist has no dedicated DB column, so it's folded
    // into notes as a readable summary (no schema migration required).
...
// ============================================================
// EDIT TRADE MODAL (item 5) — updates the trade plan only; since
// the trade is still ongoing, the balance is never touched here.
// ============================================================
...
// ---------- delete ongoing trade (item 5) — never touches balance,
// since the trade was never closed/settled. ----------
...
function attachSwipeHandlers() {
  document.querySelectorAll(".swipe-wrap").forEach((wrap) => {
    ...
    // Tapping the card again while open closes it, instead of firing
...
// Tapping SL or TP sets the closing price to that EXACT recorded
// value and lets it flow through the normal price-input handler
// (which derives the win/loss the same way a typed price would) —
// this is a convenience shortcut, not a separate calculation path.
...
// Keeps the SL/TP button highlight and the current-price label in
// sync whenever the price changes some other way (typed directly, or
// derived from a typed win/loss amount) — display only, nothing here
// computes a price.
...
// Hard guarantee, not an estimate: if the closing price is EXACTLY the
// trade's own recorded SL or TP, the result is the exact riskAmount /
// potentialProfit the trader was already shown when they opened the
// trade — returned directly, never re-derived through dpu arithmetic.
// This means the slider's SL/TP ends can never disagree with the
// Risk/Reward figures on the trade card, no matter what rounding or
// precision issue might exist anywhere else in the calculation chain.
...
document.getElementById("f_closePrice").addEventListener("input", () => {
  ...
  // Item 4 — round the closing price down to 3 decimals before using
  // it for the P&L calculation, so both the saved price and the
  // derived P&L reflect the rounded figure.
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    // Item 4 — final safety net: whatever path produced `exit`,
    // clean it of floating-point noise before it's saved (never
    // truncates real decimal precision — see roundDown3 above).
    ...
    setBusy(btn, true);
    // NOTE: two sequential writes, not one atomic transaction — acceptable for a
    // single-user MVP, but worth wrapping in a Postgres RPC function later so a
    // dropped connection between these two calls can never leave balance and
    // trade history out of sync.
...
// ============================================================
// EDIT CLOSED TRADE MODAL — one-time fix for the price a closed
// trade actually closed at (swipe-to-reveal on the Closed list).
// Mirrors the close-trade price/amount sync where a stop-loss is
// on record; otherwise the two fields are edited independently.
// The trade's balance impact is adjusted by the P&L delta, not
// recomputed from scratch, so other trades' history stays intact.
// ============================================================
...
// ============================================================
// ADD CLOSED TRADE MODAL — logs a trade straight into closed
// history without ever creating/closing an ongoing position.
// Win/loss is entered directly (not derived from SL/risk math),
// so this works even for trades with no stop-loss on record.
// ============================================================
...
// ============================================================
// WELCOME TOUR — a short spotlighted walkthrough of the home
// screen. Runs automatically once, right after first-time setup
// (works for both email and Google sign-ins, since both land on
// the same setup screen). Replayable any time from Settings.
// ============================================================
...
function renderTourStep() {
  ...
  if (!step || !target) {
    // Target isn't on screen (e.g. no weekly trades rendered a
    // slightly different layout) — skip forward instead of
    // getting stuck, or end the tour if nothing's left.
    if (tourStepIndex < TOUR_STEPS.length - 1) {
      tourStepIndex++;
      renderTourStep();
    } else {
  ...
  positionTour(target);
  // Reposition again once the smooth-scroll above has settled.
  setTimeout(() => {
